...
import { 
  Plus, Search, Shield, MoreHorizontal, Edit, Copy, Trash, 
...
export default function Dashboard() {
  ...
  const handleEdit = (id) => router.push(`/faturas/nova?id=${id}&mode=edit`);
  const handleReuse = (id) => router.push(`/faturas/nova?id=${id}&mode=clone`);

  // --- LÓGICA DE FILTRO (AGORA COM PROCEDIMENTO) ---
  const filteredInvoices = invoices.filter((invoice) => {
    ...
    const matchesProcedure = invoice.procedureTitle?.toLowerCase().includes(term); 
    ...
    <div className="min-h-screen bg-gray-50 p-8 font-sans">
      ...
      <div className="max-w-6xl mx-auto mb-8">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
                <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Faturas Proforma</h1>
                <p className="text-sm text-gray-500 mt-1">Gerencie, edite e imprima seus documentos.</p>
            </div>
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200 overflow-visible flex flex-col min-h-[500px]">
        ...
        <div className="flex-grow">
            ...
            <div className="p-16 text-center flex flex-col items-center justify-center text-gray-400 h-full">
                ...
                <p className="font-medium text-gray-600">Nenhum documento encontrado</p>
            ...
            <div className="divide-y divide-gray-100">
                ...
                const isInsurance = insuranceName.toLowerCase().trim() !== 'particular' && insuranceName.trim() !== '';

                return (
                    <div key={invoice.invoiceNumber} className="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-gray-50 transition-colors text-sm group relative">
                    ...
                    <div className="col-span-3">
                        <div className="font-medium text-gray-900 truncate pr-4" title={invoice.patientName}>
                            {invoice.patientName}
                        </div>
                        {invoice.procedureTitle && (
                            <div className="text-xs text-gray-400 truncate mt-0.5 font-normal" title={invoice.procedureTitle}>
                                {invoice.procedureTitle}
                            </div>
                        )}
                    </div>
                    ...
                    <div className="col-span-2">
                        {isInsurance ? (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-blue-50 text-blue-700 border border-blue-100 max-w-full truncate">
                                <Shield size={12} className="flex-shrink-0" /> <span className="truncate">{insuranceName}</span>
                            </span>
                        ) : (
                    ...
                    <div className="col-span-3 text-right font-medium text-gray-900">
                        {Number(invoice.grandTotal).toLocaleString('pt-PT', { minimumFractionDigits: 2 })} <span className="text-gray-400 text-xs">MT</span>
                    </div>
                    ...
                    <div className="col-span-1 flex justify-center relative">
                        <button 
                            onClick={(e) => {
                                e.stopPropagation();
                                setOpenMenuId(openMenuId === invoice.invoiceNumber ? null : invoice.invoiceNumber);
                            }}
                        ...
                        {openMenuId === invoice.invoiceNumber && (
                            <div className="absolute right-0 top-10 w-48 bg-white border border-gray-200 rounded-lg shadow-xl z-40 py-1 animation-scale-in origin-top-right">
                                ...
                                <button onClick={() => handleEdit(invoice.invoiceNumber)} className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-700 flex items-center gap-2">
                                    <Edit size={16} className="text-blue-500"/> Editar
                                </button>
                                <button onClick={() => handleReuse(invoice.invoiceNumber)} className="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-green-50 hover:text-green-700 flex items-center gap-2">
                                    <Copy size={16} className="text-green-500"/> Duplicar
                                </button>
                                <div className="my-1 border-t border-gray-100"></div>
                                <button onClick={() => handleDelete(invoice.invoiceNumber)} className="w-full text-left px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2">
        ...
        {!loading && filteredInvoices.length > 0 && (
            <div className="border-t border-gray-200 px-6 py-4 flex items-center justify-between bg-gray-50 rounded-b-xl">
                <span className="text-xs text-gray-500">
                    Mostrando {startIndex + 1} a {Math.min(startIndex + itemsPerPage, filteredInvoices.length)} de {filteredInvoices.length} resultados
                </span>
